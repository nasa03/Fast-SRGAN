    ...
    with writer.as_default():
        ...
        for x, y in dataset:
            ...
            if model.iterations % log_iter == 0:
                ...
                tf.summary.image('Low Res', tf.cast(127.5 * (x + 1.0), tf.uint8), step=model.iterations)
                tf.summary.image('High Res', tf.cast(127.5 * (y + 1.0), tf.uint8), step=model.iterations)
                tf.summary.image('Generated', tf.cast(127.5 * (model.generator.predict(x) + 1.0), tf.uint8),
                                 step=model.iterations)
                tf.summary.image('BiCubic', tf.cast(tf.image.resize((x + 1.0) * 127.5, method='bicubic'), tf.uint8), step=model.iterations)
